...
export default function CustomerProfile() {
  ...
  useEffect(() => { load(); }, [name, year, month]);
        ...
        <table>
          <thead>
            <tr>
              ...
              <th className="num">Transfer</th><th>Remarks</th><th></th>
            </tr>
          </thead>
          <tbody>
            {data.records.length === 0 && <tr><td colSpan={10} className="empty-state">No transactions in this period.</td></tr>}
            {data.records.map((r: any) => (
              <tr key={r.id}>
                <td><EditableCell value={r.created_at} onSave={(v) => updateField(r.id, 'created_at', v)} /></td>
                ...
                <td className="num">{r.sender_amount > 0 ? `→ ${r.receiver_name}` : (r.receiver_amount > 0 ? `← ${r.sender_name}` : '—')}</td>
                <td><EditableCell value={r.remarks || ''} onSave={(v) => updateField(r.id, 'remarks', v)} /></td>
                <td><button className="btn danger small" onClick={() => deleteRow(r.id)}>✕</button></td>
              </tr>
            ))}
          </tbody>
        </table>
